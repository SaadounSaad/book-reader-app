...
export class BookReaderDB extends Dexie {
  constructor() {
    ...
    this.version(1).stores({
      books: '++id, title, author, &filePath, addedAt, fileType, totalPages, fileSize',
  ...
  async addBook(book) {
    ...
    // S'assurer que les pages sont stockées sous forme de tableau de chaînes
    if (book.pages && Array.isArray(book.pages)) {
      book.pages = book.pages.map(page => {
        if (typeof page === 'object' && page !== null && page.text) {
          return page.text;
        }
